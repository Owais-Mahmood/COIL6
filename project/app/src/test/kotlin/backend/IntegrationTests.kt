...
class IntegrationTests {
    ...
    @Test
    fun `alerts endpoint returns 200`() =
        testApplication {
            setupApp(
                listOf(
    ...
    @Test
    fun `alerts endpoint returns correct count`() =
        testApplication {
            setupApp(
                listOf(
                    makeReading(alertTriggered = 1),
    ...
    @Test
    fun `site endpoint returns 200 for known site`() =
        testApplication {
            setupApp(
                listOf(
    ...
    @Test
    fun `site endpoint returns correct count`() =
        testApplication {
            setupApp(
                listOf(
                    makeReading(siteId = "site_upstream"),
    ...
    @Test
    fun `status endpoint returns correct count`() =
        testApplication {
            setupApp(
                listOf(
    ...
    @Test
    fun `alerts endpoint handles unknown site gracefully`() =
        testApplication {
    ...
    @Test
    fun `site endpoint handles sql injection style input`() =
        testApplication {
            setupApp(listOf(makeReading(siteId = "site_upstream")))
    ...
    @Test
    fun `status endpoint handles unexpected status value`() =
        testApplication {
    ...
    @Test
    fun `alerts endpoint handles empty dataset`() =
        testApplication {
            setupApp(emptyList())
            val response = client.get("/alerts")
            assertEquals(HttpStatusCode.OK, response.status)
