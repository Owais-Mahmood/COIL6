...
def turbidity_loading(csv_path: str):
...
def ring_creation(axes, value, min_value, max_value, site_id, timestampStr):
    ...
    axes.text(0.5,0.45,"NTU", ha = 'center', va = 'center', fontsize = 18, color = "#FFFFFF")
...
def plot_recent_reading(df: pd.DataFrame, output_dir: str):
    ...
    values = list(recentReading ['turbidity_ntu'])
    ...
    for i in range(len(site_ids)):
        ...
        out_path = os.path.join(output_dir, f"turbidity_{site_ids[i]}.png")
...
df = turbidity_loading(csv_path)
#range of readings from csv file
min_val, max_val = 0.0, df['turbidity_ntu'].max()
