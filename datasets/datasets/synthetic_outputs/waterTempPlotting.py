...
def waterTemp_loading(csv_path: str):
    df = pd.read_csv(csv_path)
    df['timestamp'] = pd.to_datetime(df['timestamp'])

    return df[['timestamp', 'site_id', 'wx_temp_c']]

def ring_creation(axes, value, min_value, max_value, site_id, timestampStr):
    if value > 27.0:
        ringColour = "#FF0000"
    elif value>=22.0 and value<=27.0:
        ringColour = "#FFFB00"
    elif value>5.0 and value<22.0:
        ringColour = "#0FFF0F"
    ...
    axes.text(0.5,0.45,"°C", ha = 'center', va = 'center', fontsize = 18, color = "#FFFFFF")
    ...
    if value > 27.0:
        ...
        status = 'moderate'
    elif value>10.0 and value<22.0:
        status = 'healthy'
...
def plot_recent_reading(df: pd.DataFrame, output_dir: str):
    ...
    values = list(recentReading ['wx_temp_c'])
    ...
    for i in range(len(site_ids)):
        ...
        out_path = os.path.join(output_dir, f"waterTemp_{site_ids[i]}.png")
...
df = waterTemp_loading(csv_path)
#range of readings from csv file
min_val, max_val = 0.0, df['wx_temp_c'].max()
